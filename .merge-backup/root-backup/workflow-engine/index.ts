...
export class WorkflowEngine {
  ...
  /**
   * Initialize workflow engine with vault root and register default workflows
   *
   * @param vaultRoot - Absolute path to vault root directory
   */
  async initialize(vaultRoot: string): Promise<void> {
    this.vaultRoot = vaultRoot;

    logger.info('Initializing workflow engine', { vaultRoot });

    // Import and register knowledge graph workflows
    try {
      const { registerKnowledgeGraphWorkflows } = await import('../workflows/kg/register-workflows.js');
      await registerKnowledgeGraphWorkflows(vaultRoot, this);
    } catch (error) {
      logger.warn('Failed to register knowledge graph workflows', {
  ...
  /**
   * Get vault root path (if initialized)
   */
