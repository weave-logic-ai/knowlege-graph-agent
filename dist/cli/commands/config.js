...
function createConfigCommand() {
  const config = new Command("config").description("Manage knowledge graph configuration");
  config.command("show").description("Show current configuration").option("--json", "Output as JSON").action(async (options) => {
    try {
  ...
  config.command("set <key> <value>").description("Set a configuration value").action(async (key, value) => {
    try {
      ...
      if (parts[0] === "database" && parts[1]) {
        updates.database = { path: "", [parts[1]]: parseValue(value) };
      } else if (parts[0] === "cache" && parts[1]) {
        updates.cache = { enabled: true, [parts[1]]: parseValue(value) };
      } else if (parts[0] === "logging" && parts[1]) {
        updates.logging = { level: "info", [parts[1]]: parseValue(value) };
      } else {
  ...
  config.command("reset").description("Reset configuration to defaults").option("-y, --yes", "Skip confirmation").action(async (options) => {
    try {
      const manager = createConfigManager();
      manager.reset();
      await manager.save();
  ...
  config.command("validate").description("Validate current configuration").action(async () => {
    try {
      const manager = createConfigManager();
      await manager.load();
  ...
  config.command("defaults").description("Show default configuration values").action(() => {
