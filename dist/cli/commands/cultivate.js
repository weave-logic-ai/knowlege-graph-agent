...
import { loadShadowCache } from "../../core/cache.js";
function createCultivateCommand() {
  return new Command("cultivate").description("Systematically cultivate and enhance the knowledge graph").argument("[path]", "Directory to cultivate (defaults to current directory)", ".").option("--dry-run", "Preview changes without executing", false).option("-d, --docs <path>", "Documentation path", "docs").option("-v, --verbose", "Verbose output", false).option("--seed", "Bootstrap vault with primitives from codebase analysis", false).option("--deep-analysis", "Use claude-flow agents for deep analysis", false).option("--cache", "Use shadow cache for incremental updates", true).option("--ecosystem <ecosystems>", "Filter to specific ecosystems (comma-separated)", void 0).option("--include-dev", "Include dev dependencies", false).option("--major-only", "Only process major dependencies", false).option("--refresh-cache", "Force refresh the shadow cache", false).action(async (targetPath, options) => {
...
function displaySummary(result, dryRun) {
  ...
  if (result.cache.updated > 0) {
    ...
    const hitRate = result.cache.hits + result.cache.misses > 0 ? (result.cache.hits / (result.cache.hits + result.cache.misses) * 100).toFixed(1) : 0;
    console.log(`    Hit rate: ${hitRate}%`);
  }
  console.log(`
  Duration: ${(result.duration / 1e3).toFixed(2)}s`);
