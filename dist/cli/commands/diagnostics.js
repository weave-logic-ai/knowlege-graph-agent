...
import { createHealthMonitor, createDatabaseCheck, createMemoryCheck, createDiskCheck } from "../../health/index.js";
import { createIntegrityChecker } from "../../recovery/index.js";
function createDiagnosticsCommand() {
  const diagnostics = new Command("diagnostics").alias("diag").description("System diagnostics and health monitoring");
  diagnostics.command("health").description("Run health checks on all components").option("--json", "Output as JSON").action(async (options) => {
    try {
  ...
  diagnostics.command("integrity").description("Check database integrity").option("--repair", "Attempt to repair issues").action(async (options) => {
    try {
      const cwd = process.cwd();
      const dbPath = join(cwd, ".kg", "knowledge.db");
...
Check completed in ${result.duration}ms`));
  ...
  diagnostics.command("memory").description("Show memory usage statistics").action(() => {
  ...
  diagnostics.command("status").description("Show overall system status").action(async () => {
    try {
      const cwd = process.cwd();
...
function printHealth(health) {
  ...
  console.log(chalk.gray(`Uptime: ${Math.floor(health.uptime / 1e3)}s`));
  console.log("");
  ...
  for (const component of health.components) {
    ...
    if (component.responseTime !== void 0) {
