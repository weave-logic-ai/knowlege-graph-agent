...
import { createDefaultPluginManager } from "../../plugins/manager.js";
...
function formatStatus(enabled, loaded) {
  if (loaded) {
    return chalk.green("active");
  }
  return chalk.yellow("enabled");
}
function tableRow(name, version, type, status, maxNameWidth = 30) {
  const nameCol = name.length > maxNameWidth ? name.slice(0, maxNameWidth - 3) + "..." : name.padEnd(maxNameWidth);
  return `  ${nameCol}  ${version.padEnd(10)}  ${type.padEnd(14)}  ${status}`;
}
const PLUGIN_TEMPLATE = {
  "package.json": (name, type = "analyzer") => `{
  "name": "${name}",
  "version": "1.0.0",
  ...
  "kg-plugin": {
    "type": "${type}",
    "main": "dist/index.js",
    "displayName": "${name.split("/").pop()?.split("-").map((s) => s.charAt(0).toUpperCase() + s.slice(1)).join(" ") || name}",
    "hooks": [],
    "capabilities": [],
    "configSchema": {
      "type": "object",
  ...
  "src/index.ts": (name, type = "analyzer") => {
...
function createPluginCommand() {
  ...
  command.description("Manage knowledge graph plugins").action(() => {
  ...
  command.command("list").alias("ls").description("List installed plugins").option("-p, --path <path>", "Project root path", ".").option("--json", "Output as JSON").option("-a, --all", "Show all discovered plugins including invalid ones").action(async (options) => {
    const spinner = ora("Discovering plugins...").start();
    try {
      const projectRoot = validateProjectRoot(options.path);
      const manager = createDefaultPluginManager(projectRoot);
      ...
      const plugins = options.all ? discovered : discovered.filter((d) => d.valid);
      if (options.json) {
      ...
      for (const plugin of plugins) {
        ...
        const status = plugin.valid ? formatStatus(true, manager.loader.isLoaded(name)) : chalk.red("invalid");
  ...
  command.command("install <source>").alias("add").description("Install a plugin from npm package or local path").option("-p, --path <path>", "Project root path", ".").action(async (source, options) => {
  ...
  command.command("uninstall <name>").alias("remove").description("Uninstall a plugin").option("-p, --path <path>", "Project root path", ".").option("-y, --yes", "Skip confirmation").action(async (name, options) => {
  ...
  command.command("enable <name>").description("Enable a plugin").option("-p, --path <path>", "Project root path", ".").action(async (name, options) => {
  ...
  command.command("disable <name>").description("Disable a plugin").option("-p, --path <path>", "Project root path", ".").action(async (name, options) => {
  ...
  command.command("info <name>").description("Show plugin details").option("-p, --path <path>", "Project root path", ".").option("--json", "Output as JSON").action(async (name, options) => {
...
`));
      ...
      if (kgPlugin.configSchema) {
        ...
        if (kgPlugin.configSchema.properties) {
          for (const [key, prop] of Object.entries(kgPlugin.configSchema.properties)) {
            const required = kgPlugin.configSchema.required?.includes(key) ? chalk.red("*") : "";
            const defaultVal = prop.default !== void 0 ? chalk.gray(` (default: ${JSON.stringify(prop.default)})`) : "";
  ...
  command.command("run <name> [file]").description("Run an analyzer plugin on a file").option("-p, --path <path>", "Project root path", ".").option("--stream", "Use streaming output").option("--json", "Output as JSON").option("-o, --options <json>", "Plugin options as JSON string").action(async (name, file, options) => {
    if (!file) {
    ...
    try {
      ...
      const context = manager.getPluginContext(name);
      await loadResult.plugin.initialize(context);
      const analyzer = loadResult.plugin;
      ...
      const pluginOptions = options.options ? JSON.parse(options.options) : {};
...
`));
        if (!result.success) {
          ...
          if (result.metrics) {
            ...
            if (result.metrics.entitiesFound !== void 0) {
              console.log(chalk.gray(`    Entities:      ${result.metrics.entitiesFound}`));
            }
            if (result.metrics.relationshipsFound !== void 0) {
          ...
          if (result.tags && result.tags.length > 0) {
            ...
            const tagStr = result.tags.slice(0, 20).map((t) => chalk.yellow(`#${t.value}`)).join(" ");
            console.log(`    ${tagStr}`);
          ...
          if (result.suggestions && result.suggestions.length > 0) {
            ...
            for (const suggestion of result.suggestions.slice(0, 5)) {
              const icon = suggestion.type === "add_link" ? "+" : suggestion.type === "add_tag" ? "#" : suggestion.type === "fix_format" ? "!" : "*";
  ...
  command.command("create <name>").description("Create a new plugin from template").option("-p, --path <path>", "Project root path", ".").option("-t, --type <type>", "Plugin type", "analyzer").option("-d, --dir <directory>", "Output directory", "plugins").action(async (name, options) => {
