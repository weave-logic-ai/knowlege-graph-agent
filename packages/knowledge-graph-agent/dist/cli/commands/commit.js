...
import { createAutoCommit } from "../../integrations/auto-commit.js";
...
async function interactiveCommit(git, autoCommit, status, analysis, options) {
  formatStatus(status);
...
async function performCommit(git, autoCommit, message, options) {
...
function createCommitCommand() {
  ...
  command.description("Create a git commit with optional auto-generated message").option("-m, --message <message>", "Commit message (skips auto-generation)").option("-n, --dry-run", "Show what would be committed without committing", false).option("-a, --auto", "Automatically generate commit message", false).option("-p, --push", "Push to remote after committing", false).option("--path <path>", "Repository path", ".").option("-i, --interactive", "Interactive mode for reviewing changes", false).option("-A, --all", "Stage all changes before committing", false).option("--amend", "Amend the previous commit", false).action(async (options) => {
    try {
...
`));
  ...
  command.command("status").description("Show git status with analysis").option("--path <path>", "Repository path", ".").action(async (opts) => {
    try {
...
`));
  ...
  command.command("log").description("Show recent commits").option("--path <path>", "Repository path", ".").option("-n, --count <count>", "Number of commits to show", "10").action(async (opts) => {
    try {
      const projectRoot = validateProjectRoot(opts.path);
      const git = createGitClient({ workingDirectory: projectRoot });
      if (!await git.isRepo()) {
...
`));
  ...
  command.command("diff").description("Show diff of changes").option("--path <path>", "Repository path", ".").option("-s, --staged", "Show only staged changes", false).option("-f, --file <file>", "Show diff for specific file").action(async (opts) => {
    try {
      const projectRoot = validateProjectRoot(opts.path);
      const git = createGitClient({ workingDirectory: projectRoot });
      if (!await git.isRepo()) {
