...
export type VectorStoreEvent =
  | { type: 'insert'; id: string; timestamp: Date }
  | { type: 'delete'; id: string; timestamp: Date }
  | { type: 'search'; queryId: string; resultCount: number; durationMs: number; timestamp: Date }
  | { type: 'rebuild'; reason: string; timestamp: Date }
  | { type: 'error'; error: Error; operation: string; timestamp: Date };
...
export interface IVectorStore {
  ...
  insert(entry: { id: string; vector: number[]; metadata?: Record<string, unknown> }): Promise<void>;

  /** Batch insert multiple vectors */
  batchInsert(operation: BatchInsertOperation): Promise<BatchInsertResult>;

  /** Search for similar vectors */
  search(query: { vector: number[]; k?: number; filter?: Record<string, unknown>; minScore?: number }): Promise<SearchResult[]>;
