...
1. **Adaptive Depth Retrieval**
   ...
   - Example: "How do we handle authentication?" → Start at level_2 (semantic patterns)
...
3. **Progressive Refinement**
   ...
   - User asks for more detail → descend to level_1 or level_0
   - User asks for summary → ascend to higher level
...
| **Granularity** | Fixed chunking (paragraphs) | Multi-level (atomic → strategic) |
...
| **Abstraction** | Single level | Hierarchical (details → patterns → principles) |
...
def decompose_and_route(complex_query):
    ...
    # Result:
    # 1. "authentication issues last month" → temporal + semantic
    # 2. "what did we learn" → semantic (level_2 patterns)
    # 3. "for next time" → preference-weighted recommendations
...
├─────────────────────────────────────────────────────────────┤
│  • Experience Capture (task execution, user feedback)         │
│  • Metadata Extraction (LLM-powered)                          │
│  • Relationship Detection (causal, temporal, semantic)        │
...
├─────────────────────────────────────────────────────────────┤
│  • Semantic Embeddings (content)                              │
│  • Temporal Embeddings (time-aware)                           │
│  • Preference Embeddings (user-specific)                      │
│  • Structural Embeddings (graph position)                     │
│  • Composite Fusion (learned combination)                     │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
...
├─────────────────────────────────────────────────────────────┤
│  Level 0: Atomic Actions (Vector DB + Time-series DB)         │
│  Level 1: Episodic Tasks (Vector DB + Graph DB)               │
│  Level 2: Semantic Patterns (Graph DB + KG)                   │
│  Level 3: Strategic Principles (KG + Document Store)          │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
...
├─────────────────────────────────────────────────────────────┤
│  • Pattern Detection (identify recurring sequences)           │
│  • Knowledge Distillation (generalize from experiences)       │
│  • Hierarchy Building (create abstractions)                   │
│  • Memory Pruning (archive low-value memories)                │
│  • Relationship Refinement (strengthen/weaken links)          │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
...
│  • Multi-Strategy Search (semantic + keyword + temporal +     │
│    graph + preference)                                        │
│  • Adaptive Routing (query-type → best strategy)              │
│  • Result Fusion (rank aggregation)                           │
│  • Contextual Experience Replay (relevant past experiences)   │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
...
│  • Preference Embedding Learning (VPL approach)               │
│  • Preference Dimensionality Reduction (PCA/DRM)              │
...
├─────────────────────────────────────────────────────────────┤
│  1. Execute Task                                              │
│  2. Capture Experience → Level 0 Memory                       │
│  3. Collect User Feedback → Preference Update                 │
│  4. Retrieve Relevant Past Experiences (CER)                  │
│  5. Apply Learned Patterns                                    │
│  6. Record Outcome                                            │
│  7. Periodic Consolidation → Level 1, 2, 3 Memories           │
│  8. Adapt Retrieval Strategies based on success               │
└─────────────────────────────────────────────────────────────┘
```
...
10. **Build hierarchically** - atomic → episodic → semantic → strategic
...
2. **Hierarchical organization** (atomic → episodic → semantic → strategic)
