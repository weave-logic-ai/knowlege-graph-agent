...
import ora from 'ora';
async function executePerformanceAnalysis(action, options) {
    console.log(chalk.bold.blue('\n🚀 SOP-008: Performance Analysis Workflow\n'));
    console.log(chalk.white(`Action: ${action}`));
    if (options.target)
        console.log(chalk.white(`Target: ${options.target}`));
    if (options.baseline)
        console.log(chalk.white(`Baseline: ${options.baseline}`));
    console.log();
    const spinner = ora();
    try {
        if (action === 'analyze') {
