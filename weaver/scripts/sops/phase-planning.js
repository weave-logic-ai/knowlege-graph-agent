...
import ora from 'ora';
async function executePhasePlanning(phaseId, options) {
    ...
    if (options.teamSize)
        console.log(chalk.white(`Team Size: ${options.teamSize}`));
    if (options.sprints)
        console.log(chalk.white(`Sprints: ${options.sprints}`));
    console.log();
    const spinner = ora();
    try {
...
//# sourceMappingURL=phase-planning.js.map